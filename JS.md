...
Определение Properties в React-компоненте это по сути спецификация и внещний интерфейс.

Любой читающий код компонента, должен в первую очередь изучить интерфейс, прежде чем изучать содержимое.

Таким образом `propTypes` должны быть на видном месте.
